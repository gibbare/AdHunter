"""
Kamerastore scraper – kamerastore.com/second-hand
HTML-scraping via BeautifulSoup.
"""
import requests
...
def search(query: str, max_price: Optional[int] = None, min_price: Optional[int] = None) -> list[dict]:
    url = f"{BASE}/second-hand?search={requests.utils.quote(query)}"
    try:
    ...
    # Produktkort – anpassas efter faktisk HTML-struktur
    for card in soup.select("div.product-card, article.product, .product-item, li.product"):
        title_el = card.select_one("h2, h3, .product-title, .product-name, a[title]")
