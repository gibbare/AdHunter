"""
Cyberphoto scraper – cyberphoto.se/begagnat
"""
import requests
...
def search(query: str, max_price: Optional[int] = None, min_price: Optional[int] = None) -> list[dict]:
    url = f"{BASE}/sok?q={requests.utils.quote(query)}&used=1"
    try:
    ...
    for card in soup.select(".product-card, .product, .item, article"):
        title_el = card.select_one("h2, h3, .product-name, a[title]")
